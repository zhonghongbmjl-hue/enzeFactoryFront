...
import { createPlanningApi } from './index'

describe('production planning api', () => {
  it('keeps decimal quantities as strings and sends a stable idempotency key', async () => {
    const get = vi.fn().mockResolvedValue(mockApiResponse({ id: 'plan-1' }))
    const post = vi.fn().mockResolvedValue(mockApiResponse({ id: 'plan-1', version: 0 }))
