...
import { createProcurementApi } from './index'

describe('procurement api', () => {
  it('loads both material branches and sends versioned idempotent actions', async () => {
    const get = vi
      .fn()
      .mockResolvedValue(mockApiResponse({ salesOrderId: 'order-id', branches: [] }))
    const post = vi
      .fn()
      .mockResolvedValue(mockApiResponse({ id: 'plan-id', status: 'PENDING_APPROVAL', version: 2 }))
