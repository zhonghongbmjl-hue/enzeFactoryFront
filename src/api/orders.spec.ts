...
import { createSalesOrderApi } from './orders'

describe('sales order api', () => {
  it('forwards paging and uses idempotent lifecycle endpoints', async () => {
    const get = vi.fn().mockResolvedValue({
      data: { data: { content: [], totalElements: 0, totalPages: 0, page: 1, size: 25 } },
    })
    const post = vi.fn().mockResolvedValue({ data: { data: { id: 'order-id', version: 4 } } })
  ...
  it('uploads multipart delivery evidence before sending the versioned manual confirmation', async () => {
    const post = vi.fn().mockResolvedValue({ data: { data: { id: 'fact-id' } } })
