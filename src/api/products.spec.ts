...
import { createBomApi, createProductApi } from './products'
...
describe('product api', () => {
  ...
  it('uses named BOM lifecycle endpoints with a version and idempotency header', async () => {
    const post = vi.fn().mockResolvedValue({ data: { data: { status: 'PENDING_APPROVAL' } } })
    const get = vi.fn().mockResolvedValue({
      data: { data: { content: [], page: 2, size: 25, totalElements: 0, totalPages: 0 } },
    })
